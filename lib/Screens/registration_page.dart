...
class _SignUpState extends State<SignUp> {
  ...
  addError(String?error){
    if(!errorList.contains(error)){
     setState(() {
       print("error add${errorList.toString()}");
       errorList.add(error);
     });
    }
  }
  ...
  removeError(String?error){
    if(errorList.contains(error)){
      setState(() {
        print("error remove ${errorList.toString()}");
...
class ErrorList extends StatelessWidget {
  ErrorList({Key? key, required this.error}) : super(key: key);
final  error;
  @override
  Widget build(BuildContext context) {
    return Column(
      children: List.generate(error.length, (index) =>Text(error[index]))
    );
  }
}
